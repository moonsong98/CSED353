...
void TCPReceiver::segment_received(const TCPSegment &seg) {
    ...
    if(seg.header().fin)
        recievedFIN = true;
    // Calculate absolute sequence number to save in buffer, as in a document, checkpoint is the index of the last reassembled byte
...
optional<WrappingInt32> TCPReceiver::ackno() const { 
    ...
    // If FIN is recieved but some segment in between SYN and FIN is not recieved, do not add 1 since last segment is not assembled yet
